...
############必须要补充的参数，没有办法的
    ...
    def get_nest_number_and_name(self,index):
        ...
        #########在这个里面或许可以搞定这个事情的
        model_number=self.model_viewer[index].linedit.text()

        model_name=self.model_viewer[index].model_name
        ...
        self.model_viewer[index].start_nest_new(model_number,model_name,self.sheet_size_width,self.sheet_size_height,\
        self.nest_start_x,self.nest_start_y,self.old_model_height,\
        ...
        if(self.left_rectangle!=0):
            ...
            print("该打印信息位于layout6的顶层,self.nest_start_x,self.nest_start_y的值为",\
                self.nest_start_x,self.nest_start_y,"model_name的值为",model_number,\
                    "model_name的值为",model_name,"原来模型的宽度为",self.old_model_width,\
        ...
        self.cad_window.open_dxf_file("new_nest.dxf") 
