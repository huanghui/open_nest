...
from polycrash03 import polyCrash,polyCrash2,polyCrash3,polyCrash_3,polyCrash_4

from maxminpoint02 import findMaxXVertex,findMaxYVertex,findMinXVertex,findMinYVertex


########引入排版函数好一点。
from model_copy_move import copy_num_dxf,copy_dxf
...
def save_as(sdxf,savename):
    base_dxf = ezdxf.readfile(sdxf)
...
#def nestWidth2(n,srcdxf,height,concave):
def nestWidth3_old(nest_whole_number,row_number_of_nest,horizantal_number_of_nest,srcdxf,height,concave):
...
###########开始横向排版
    ...
    #msp.add_text("Hello World") # 添加文字
        doc.saveas("D4.dxf") # 保存图形
        "这里的save的d4是在whole_number大比x_number大的情况下，不要乱碰"
    ...
    #msp.add_text("Hello World") # 添加文字
        ...
        #saveas("D2.dxf")
            ...
            ######怕D4被污染了，这里删除重新来
            print("如果模型的数量为偶数且只有一行，那么进入这里处理")
        ...
        #saveas("D2.dxf")
            ...
            if(os.path.exists("Dy.dxf")):
                os.remove("Dy.dxf")
                exceptiondxf = ezdxf.readfile("D4.dxf")
                exceptiondxf.saveas("Dy.dxf")
...
def nest_with_rectangle(dxf_model,number_for_nest,width,height):
    ...
    #########这里model_height必须要重新赋值，不然会出大问题，因2个模型组合的话，会产生高度变化
    model_Height=findMaxYVertex("edge.dxf")[1]-findMinYVertex("edge.dxf")[1]
...
def nest_with_rest_old(dxf_model,number_for_nest,width,height,model_Width,model_Height):
    print("需要排版的模型名字为",dxf_model,"排版的数量为",number_for_nest,"版面的宽和长度为",width,height)
    ##########需要测量模型的长款和调整模型的位置，这是个大工程哦。
    ###先要调整模型的角度
    ...
    print("num_of_width的值为",num_of_width)
    #######确定可以排多少行，比如排版100，每行12，那么就是100/12=8+1为9行。
    if(num_of_width>number_for_nest or num_of_width==number_for_nest):
        num_of_height=1
    else:
        num_of_height=(int(number_for_nest)/num_of_width)+1

    print("num_of_height的值为",num_of_height,'int(number_for_nest/num_of_width的值为',int(number_for_nest/num_of_width))
    print("numer_for_nest/num_of_width",(number_for_nest/num_of_width))
    #print("模型的横排可以的值为",num_of_width,"模型的宽度为",model_Width)
   # print("模型的竖排值为",num_of_height)
    ##########
    ###########如果起始的排版点不等于0，那么问题大了，
...
###########
def nest_with_rest(dxf_model,number_for_nest,width,height,model_Width,model_Height,nest_distance):
    ...
    model_Width=model_Width
     #####此处model_width没有什么变化。
    num_of_width=int(width/model_Width)

        #num_of_width=num_of_width-1
    two_compound_width=findMaxXVertex("edge.dxf")[0]-findMinXVertex("edge.dxf")[0]
    num_of_compound_line=2*int(width/two_compound_width)

    #print("")
    num_of_width=max(num_of_width,num_of_compound_line)
    ...
    if(num_of_width>number_for_nest or num_of_width==number_for_nest):
        ...
        if(number_for_nest%num_of_width ):
            num_of_height=(int(number_for_nest/num_of_width))+1
        else:
            num_of_height=(int(number_for_nest/num_of_width))
    print("num_of_height的值为",num_of_height,'int(number_for_nest/num_of_width的值为',int(number_for_nest/num_of_width))
    print("numer_for_nest/num_of_width",(number_for_nest/num_of_width))
    #model_Height=findMaxYVertex("edge.dxf")[1]-findMinYVertex("edge.dxf")[1]
    #print("组合模型的最终最适合的高度是",model_Height)
    if(num_of_height*model_Height>height+num_of_height*nest_distance+0.1):
        print("排版的矩阵异常！！！！！！！！！！，nest_with_rest")
        print("该模型的排版数量超过了版面容纳的最大数量，请重新排版")
        return 0,0

    else:
        ###########修正版面边缘留出一定的边缘宽度
        width=width-nest_distance
        num_of_width,num_of_height=nestWidth3(width,number_for_nest,num_of_width,num_of_height,"edge.dxf",model_Height,0,nest_distance)

    print("最后mix成功","num_of_width得值为",num_of_width,"模型排版的层数为",num_of_height)
    return num_of_width,num_of_height
    
#########################
#########################
#def nestWidth3(nest_whole_number,row_number_of_nest,horizantal_number_of_nest,srcdxf,height,concave):
def nestWidth3(paper_width,nest_whole_number,row_number_of_nest,horizantal_number_of_nest,srcdxf,height,concave,dx):
    
#############dx代表模型的排版间隔，暂时写死，不作更改
    #dx=2
    whole_number=int(nest_whole_number)
...
###########我觉得放这里判断是不是很不好？但是动了这个，怕有其他问题。
    ...
    x_number=int((2*paper_width)/width)
        ##########这里还有这个问题
    if(x_number%2 and (x_number*width)>2*paper_width ):
        x_number=x_number-1

    ######这里需要验证，不然就容易出错,这里索性去掉一个，因为涉及到大量的配置的模型，去掉一个更为
    #######这里验证不正确，应该是第一行排完以后验证。

    y_number=int(nest_whole_number/x_number)

    #####
    #if(y_number*height>rect_height):

    #row_number_of_nest=x_number
    
    print("在nestwidth3的排版中x_number的数量为",x_number,"在最终的排版中y_number的数量为",y_number)
    #print("2组合模型的整体宽度为",width,"单排更新的数量为",x_number)
    
    ###########在正式开始排版前还需要就竖直方向测试高度
    moveydxf(srcdxf,height).saveas("Dheight.dxf")
    mix("Dheight.dxf",srcdxf,"Dhtest.dxf")
    distanceH=polyCrash3("Dhtest.dxf")
    while(distanceH==0):
        os.remove("Dhtest.dxf")
        height=height+1
    ...
    if(whole_number>x_number):
        '''如果是奇数'''
        print("whole_number的值为",whole_number,"x_number的值为",x_number)
        if(x_number%2):

            copy_num_dxf(srcdxf,width,0,int((x_number-1)/2),"D0.dxf")
            ...
            ####如果是偶数？
            copy_num_dxf(srcdxf,width,0,int((x_number)/2),"D0.dxf")

            row_max_x=findMaxXVertex("D0.dxf")[0]-findMinXVertex("D0.dxf")[0]
            if(row_max_x>1500):
                x_number=x_number-1
    ...
    #msp.add_text("Hello World") # 添加文字
        ...
        if(os.path.exists("D2.dxf")):
            os.remove("D2.dxf")
        if(whole_number%2):
            print("如果模型的数量为奇数且只有一行，那么进入这里处理")
            copy_num_dxf(srcdxf,width,0,int(whole_number/2),"D2.dxf")
            mix("D4.dxf","D2.dxf","D4.dxf")
            '''for  i in range(0,int(whole_number/2)):
                movexdxf(srcdxf,i*width).saveas("D2.dxf")
        #saveas("D2.dxf")
...
############对其进行竖直排版
        ########竖直方向不能碰撞，所以需要设置
    #delta_y=findMaxYVertex("Dy.dxf")
    #delta_x=findMaxYVertex("Dy.dxf")
    
    print("y_number的数量是多少？",y_number)
    if(y_number>1 or y_number==1):
        #if( y_number==1):
            #copy_num_dxf("D4.dxf",0,height,y_number,"D3.dxf")

        #if(y_number*height>)
       # else:
        copy_num_dxf("Dy.dxf",0,height,y_number,"D3.dxf")
        mix("D4.dxf","D3.dxf","D4.dxf")
        os.remove("D3.dxf")
        '''for j in range(0,y_number-1):
        ...
        #saveas("D2.dxf")
            ...
            #else :   #(j==horizantal_number_of_nest-1):
    if(y_number!=0):        
        rest_number=whole_number-x_number*y_number+rest_number
    else:
        rest_number=whole_number+rest_number

#######如果rest_number或者rest_number只需要排一行则
    #print("horizantal_number_of_nest也即竖排的数量为",horizantal_number_of_nest,)
    if(rest_number!=0 and rest_number!=whole_number):
        print("最后剩下需要排版的数量为",rest_number)
        #if(rest_number%2):
        copy_num_dxf(srcdxf,width,0,int((rest_number)/2),"D2.dxf")
        #moveydxf("D2.dxf",(y_number-1)*height).saveas("D3.dxf")
        moveydxf("D2.dxf",y_number*height).saveas("D3.dxf")
        mix("D4.dxf","D3.dxf","D4.dxf")
        ...
        #saveas("D2.dxf")
            #mix("D0.dxf","D2.dxf","D0.dxf")
        if(rest_number%2):
            #copy_num_dxf("3.dxf",((rest_number-1)/2)*width,(y_number-1)*height,1,"D3.dxf")
            movexdxf("3.dxf",((rest_number-1)/2)*width).saveas("D2.dxf")
            moveydxf("D2.dxf",(y_number)*height).saveas("D3.dxf")
            mix("D4.dxf","D3.dxf","D4.dxf")
        ...
        print("")
        print("nestwidth3中的x_number的值为",x_number)
        print("")
...
if __name__=="__main__":
   ...
   #classifyForDxf("Drawing21.dxf")
    nest_with_rectangle("Piece21-S7.dxf",10,1500,6000)
    
    nest_with_rectangle("Piece21-S7.dxf",10,1500,6000)
    #nest_with_rest("model13.dxf",10,1500,6000,model_Width,model_Height,nest_distance):
